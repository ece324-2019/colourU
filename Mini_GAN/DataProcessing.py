...
def import_folder(path, num_imgs=-1, start = 0, expected_size=(64, 64, 3), shuffle=False):
    ...
    return torch.tensor(rgb2lab(np.stack(img_array_stack, axis=2))).permute(2, 3, 0, 1).float()  #/127
...
def display_processed_imgs (img_tensor, gray_img_tensor, num_imgs):
    ...
    for i in range(num_imgs):
        img = img_tensor[i, :, :, :].permute(1, 2, 0)  #*127
        fig.add_subplot(2, num_imgs, i+1)
        plt.imshow(lab2rgb(img, illuminant = "D50"))
        img = gray_img_tensor[i, :, :, :].permute(1, 2, 0)  #*127
